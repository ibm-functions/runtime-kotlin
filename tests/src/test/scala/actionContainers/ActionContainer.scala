...
object ActionContainer {
  ...
  private def awaitDocker(cmd: String, t: Duration): (Int, String, String) = {
    System.out.println("Calling awaitDocker for $cmd")
  ...
  def withContainer(imageName: String, environment: Map[String, String] = Map.empty)(code: ActionContainer => Unit)(
    ...
    try {
      ...
      Thread.sleep(100)
      val (_, out, err) = awaitDocker(s"logs $name", 30 seconds)
      (out, err)
    } finally {
      awaitDocker(s"kill $name", 30 seconds)
      awaitDocker(s"rm $name", 30 seconds)
    }
  }
